  ...
  * */
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      ...
      case x::Nil => trees
      case x:: xs => {
        val newFork = Fork(x, xs.head, chars(makeCodeTree(x, xs.head)), weight(makeCodeTree(x, xs.head)))
        (newFork :: xs.tail).sortWith((a, b) => weight(a) < weight(b))
      }
   ...
   */
    def until(xxx: List[CodeTree] => Boolean, yyy: List[CodeTree] => List[CodeTree])(zzz: List[CodeTree]): CodeTree = {
      if (xxx(zzz)) zzz.head // if zzz is a singleton, then return the only element in zzz
      else until(xxx, yyy)(yyy(zzz)) // if zzz is not a singleton, then recursively combine zzz
   ...
   */
    def createCodeTree(chars: List[Char]): CodeTree = {
      ...
      val orderedLeaftList = makeOrderedLeafList(freq)
      until(singleton, combine)(orderedLeaftList)
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def move(t: CodeTree, b: List[Bit]): List[Char] = t match {
        case Leaf(ch, _) => if (b.isEmpty) List(ch) else ch :: move(tree, b.tail)
        case Fork(l, r, _, _) => if (b.head == 0) move(l, b.tail) else move(r, b.tail)
      }
      move(tree, bits)
      /*
      tree match {
        case Leaf(ch, _) => if (bits.isEmpty) List(ch) else ch :: decode(tree, bits.tail)
...
*/
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def lookup(t: CodeTree)(ch: Char): List[Bit] = t match {
        case Leaf(_, _) => Nil
        case Fork(l, r, _, _) => if (chars(l).contains(ch)) 0 :: lookup(l)(ch)
                                  else 1 :: lookup(r)(ch)
      }
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      a.map{case (c, list) => (c, 0::list)} :::  b.map{case (c, list) => (c, 1::list)}
    }
